...
\node[app box] (handle) {%
  ...
  {\footnotesize \texttt{SocketServer} / \texttt{SocketClient} with shared endpoint configuration}
};

\node[app box, below=7mm of handle] (registration) {%
  \textbf{\textcolor{snodecTitleGreen}{Activation call}}\\[-0.4mm]
  {\footnotesize Each \texttt{listen(...)} creates a listening flow}\\[-0.2mm]
  {\footnotesize Each \texttt{connect(...)} creates a connecting flow}
};
...
\node[split box, left=7mm of splitCenter] (server) {%
  \textbf{\textcolor{snodecTitleBlue}{Server-side flow}}\\[-0.4mm]
  {\footnotesize uses the server instance's policy}
};

\node[split box, right=7mm of splitCenter] (client) {%
  \textbf{\textcolor{snodecTitleBlue}{Client-side flow}}\\[-0.4mm]
  {\footnotesize uses the client instance's policy}
};
...
\node[runtime box, below=12mm of splitBottom] (machinery) {%
  ...
  {\footnotesize runtime advances acceptance or connection attempts}
};
