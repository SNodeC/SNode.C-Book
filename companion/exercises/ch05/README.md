...
ctest --test-dir build/labs -R '^exercise-ch05-deferred-work$' --output-on-failure -V
```
...
```sh
ctest --test-dir build/labs -R '^exercise-ch05-model-checkpoint$' --output-on-failure -V
```

The target reuses `companion/exercises/ch30/model-ownership.cpp` and compiles the
