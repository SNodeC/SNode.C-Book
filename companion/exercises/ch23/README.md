...
ctest --test-dir build/labs -R '^exercise-ch23-durable$' --output-on-failure -V
```
...
```sh
ctest --test-dir build/labs -R '^exercise-ch23-error$' --output-on-failure -V
```
...
observation (`exercise-ch22-unavailable-output`). It shows accepted in-memory state
