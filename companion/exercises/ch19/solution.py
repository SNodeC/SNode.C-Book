...
from lab_support import connect, environment, free_port, running, wait_log
...
def run(mode, server, other):
    if mode == 'checkpoint':
        with tempfile.TemporaryDirectory() as config:
            subprocess.run([sys.executable, str(Path(__file__).resolve().parents[1] / 'ch18/observers.py'), other],
                           env=environment(config), check=True, timeout=150)
    port = free_port()
    with running(server, ['--log-level=6', 'legacy', 'local', '--host=127.0.0.1', f'--port={port}']) as (process, log):
        if mode == 'negotiation':
            ...
            wait_log(log, 'WebSocket echo server connected')
