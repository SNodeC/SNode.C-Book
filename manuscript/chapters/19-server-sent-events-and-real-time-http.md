...
SSE is a different fit from WebSocket, not a weaker version of it. It fits cases where the server should push events and the client does not need to send messages back over the same long-lived channel. If both sides need to send independent messages over one long-lived channel, WebSocket becomes the more natural fit. Chapter 20 treats that case.
...
SSE retry information updates the underlying client reconnect/retry configuration introduced in Chapter 16.
...
That connects directly to the diagnostic model from Chapter 14 and the configuration/timing model from Chapter 16. SSE does not need a new operational philosophy; it applies the existing one to a long-lived HTTP stream.
...
Public solutions and lab commands: `companion/exercises/ch19/README.md`.
:::
