...
This diagram connects Chapters 7, 9 and 10. Chapter 7 explained the server/client/connection relationship. Chapter 9 explained the context as the per-connection protocol endpoint. This chapter explains the construction step between connection and context.
...
This rule follows directly from Chapter 9. If the context is one per-connection protocol endpoint, the factory must preserve that relationship.
...
Chapter 9 said that a context may depend on application services, but those dependencies should be visible. Chapter 10 shows the construction boundary where such dependencies can be passed deliberately.
...
### The factory as the Chapter 11 bridge
