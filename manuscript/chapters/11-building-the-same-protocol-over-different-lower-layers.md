...
This is where the design discipline from Chapters 9 and 10 becomes visible across lower families. The protocol does not become independent of all lower-layer reality.
...
This is the main reason Chapter 9 emphasized context discipline.
...
Chapters 9 and 10 came first because a well-factored context and a disciplined factory make lower-family transfer practical. The factory does not make the protocol portable by magic. It keeps the construction boundary clean enough that portability can be judged clearly.
...
That is the central transfer lesson from Chapters 6–8. The framework model can remain stable while the meaning of an endpoint changes. A good design does not hide that change. It places it where it belongs.
...
This follows directly from Chapter 9. A context is the per-connection protocol endpoint. If a value describes the state of this conversation with this peer, the context is often the right scope for it.
...
Construction-time variation is healthy when it makes the protocol roles clear. It becomes unhealthy when the factory begins to execute the protocol conversation. That boundary was the main lesson of Chapter 10, and it becomes especially useful when the same protocol travels across carriers.
