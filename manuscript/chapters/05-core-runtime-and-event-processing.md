...
\SNodeCNextSectionMark{\thesection. DESCRIPTOR EVENT RECEIVERS}
...
Chapter 4 introduced retry and reconnect behavior as operational concerns of configured instances. In the stream flow-controller path, that now becomes concrete. Retry and reconnect need time, and the runtime already has first-class timer support.
...
The public application control surface remains `core::SNodeC`. The graph above explains internal coordination; it does not ask an installed consumer to include private `EventLoop` or multiplexer implementation headers. Chapters 14 and 15 connect the same shutdown model to TLS, timeouts, and output pressure.
