...
This mirrors the layer model from Chapter 5. The application shape may stay recognizable, but the endpoint family still matters.
...
Compare the IPv4 and IPv6 echo selections below while holding the factory and protocol behavior fixed. A successful exchange on each loopback address will test the claim that the protocol does not depend on the network family more directly than two similar type names. A failure on one family will then be a reason to inspect that family's binding and reachability, before changing the shared protocol.
...
So the convenience API is not a separate communication model. It configures the handle and then enters the same registration path that Chapter 8 will examine.
...
The local point is the family selection. Chapter 27 gives the consolidated include/component matrix.
...
Chapter 5 introduced `legacy` and `tls` as connection-layer variants. Unix domain sockets fit into that same layer story.
