...
A server or client handle gives application code access to an instance. Its activation flows use the addresses from Chapter 7 to listen or connect; a successful connection then has its own lifetime.
...
The same separation explains coordinated shutdown: reader and writer notifications belong to one complete connection cleanup, even though they arrive through different receiver subobjects. Chapter 14 uses these identities to interpret log records; Chapter 29 uses them to interpret lifecycle tests.
