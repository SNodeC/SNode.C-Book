...
The broker's outgoing publication and the subscriber's callback provide the next observations. Retain the actual topic and payload at the subscriber, not only a publisher-side “sent” message. For this experiment, the expected result is an exact match with the selected value on the selected topic. That observation demonstrates this broker-mediated path for this run. Durable storage, recovery after restart and processing by other interested subscribers each need their own experiment and observed result.
