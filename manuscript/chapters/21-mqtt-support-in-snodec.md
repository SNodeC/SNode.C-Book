...
Native MQTT combines a stream `SocketContext` with `MqttContext`; MQTT over WebSocket combines a WebSocket subprotocol role with the same MQTT-facing bridge. The former carries MQTT directly over a stream, while the latter adds HTTP upgrade and WebSocket framing. Chapter 22 develops that carrier choice.
