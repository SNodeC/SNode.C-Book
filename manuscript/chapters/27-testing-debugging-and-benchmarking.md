...
Inspect peer-received bytes in HTTP component tests, separately from diagnostic presentation (Chapters 13 and 16). Malformed, boundary-sized and fragmented parser inputs must distinguish accepted, rejected and pending data; a small complete-input example cannot establish that contract.
...
They do not equate one write with a packet or callback; Chapters 6 and 11 depend on that distinction.
...
Test the installed environment described in Chapter 26: loader paths, modules, configuration, service identity, protected TLS material, database access and package dependencies can differ from development conditions.
...
<!-- snodec-source: companion/exercises/ch27/roundtrip.py -->
```python
import math
import socket
import statistics
