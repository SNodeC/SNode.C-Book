...
TLS adds timed phases to the connection lifecycle. Chapter 14 described TLS startup as the handshake phase and TLS shutdown as close-notify/shutdown behavior. Chapter 15 treats them as part of the broader time model. TLS initialization and TLS shutdown can both need time bounds. Both can fail. Both belong to the connection layer.
