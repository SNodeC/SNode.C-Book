#!/usr/bin/env python3
"""Recheck the Part VI budgets, teaching contract, scope, preservation and artifacts."""
...
base = subprocess.check_output(['git', 'rev-parse', ':/proposal-readiness: phase 5f — refine Part V and verify configuration diagnostics'], text=True).strip()
...
assert metrics == json.loads((REVIEW / 'metrics-after-phase-5g-follow-up.json').read_text())
baseline = json.loads((REVIEW / 'metrics-after-phase-5f.json').read_text())
...
allowed = {'manuscript/chapters/14-tls-across-the-framework.md',
           'manuscript/chapters/15-timeouts-retries-and-failure-modes.md',
           'manuscript/parts/part-06-secure-and-robust-communication.md'}
...
assert 'Part VI checkpoint' in (ROOT/'manuscript/chapters/15-timeouts-retries-and-failure-modes.md').read_text()
assert 'Part VI checkpoint' in (ROOT/'companion/exercises/ch15/README.md').read_text()
...
for path in subprocess.check_output(['git','ls-tree','-r','--name-only',base,'companion/examples','companion/exercises'],text=True).splitlines():
    if Path(path).suffix in {'.cpp','.h','.py'} and path != 'companion/exercises/ch14/tls.py': assert (ROOT/path).read_text()==old(path),path
...
assert all(value == 0 for value in json.loads((REVIEW/'phase-5g-follow-up-results.json').read_text()).values())
assert '100% tests passed, 0 tests failed out of 36' in (REVIEW/'phase-5g-follow-up-labs.log').read_text()
t=metrics['totals']
...
with tarfile.open(ROOT/'dist/packages/snodec-book-proposal-package.tar.gz') as archive:
    ...
    assert any(m.name.endswith('/ch15/CMakeLists.txt') for m in files)
result={'chapters':chapters,'part_vi_words':sum(c['words_after'] for c in chapters),'part_vi_budget':6350,
        ...
        'word_delta_from_phase_5f':t['total_words']-baseline['totals']['total_words']}
(REVIEW/'phase-5g-follow-up-exit-checks.json').write_text(json.dumps(result,indent=2)+'\n')
print(json.dumps(result,indent=2))
print('PASS: Part VI budgets, pedagogy, mappings, scope, preserved listings, clean final PDFs and package contents.')
print('Execution and editorial/visual review are recorded in separate Phase 5g logs; future Parts are not certified.')

for name in ['tls-runtime.cpp', 'run-tls.py']:
    prior = subprocess.check_output(['git', 'show', 'HEAD:review/verification/refinement-probes/' + name], text=True) if (ROOT/'review/verification/refinement-probes'/name).exists() else old('review/verification/refinement-probes/' + name)
    assert (ROOT/'companion/exercises/ch14'/name).read_text() == prior
