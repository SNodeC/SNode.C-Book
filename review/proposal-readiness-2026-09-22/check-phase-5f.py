...
assert all(value == 0 for value in json.loads((REVIEW/'phase-5f-final-results.json').read_text()).values())
assert '100% tests passed, 0 tests failed out of 32' in (REVIEW/'phase-5f-final-labs.log').read_text()
t=metrics['totals']
...
(REVIEW/'phase-5f-exit-checks.json').write_text(json.dumps(result,indent=2)+'\n')
