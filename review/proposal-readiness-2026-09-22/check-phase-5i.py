#!/usr/bin/env python3
"""Recheck the Part VIII budgets, teaching contract, scope, preservation and artifacts."""
...
base = subprocess.check_output(['git', 'rev-parse', ':/proposal-readiness: phase 5h — refine Part VII and verify web protocol boundaries'], text=True).strip()
...
assert metrics == json.loads((REVIEW / 'metrics-after-phase-5i.json').read_text())
baseline = json.loads((REVIEW / 'metrics-after-phase-5h.json').read_text())
...
allowed = {'manuscript/chapters/20-mqtt-support-in-snodec.md', 'manuscript/chapters/21-mqtt-over-websocket.md', 'manuscript/chapters/22-designing-iot-systems-with-multiple-protocols.md', 'manuscript/parts/part-08-iot-and-message-oriented-systems.md'}
...
chapters=[]
for n,budget,headings in [(20,2350,5),(21,2100,4),(22,2950,8)]:
...
assert 'Part VIII checkpoint' in (ROOT/'manuscript/chapters/22-designing-iot-systems-with-multiple-protocols.md').read_text()
assert 'Part VIII checkpoint' in (ROOT/'companion/exercises/ch22/README.md').read_text()
...
regeneration=next(line for line in (ROOT/'build/proposal-readiness-phase-5i-examples/build.ninja').read_text().splitlines()
...
assert all(value == 0 for value in json.loads((REVIEW/'phase-5i-final-results.json').read_text()).values())
assert '100% tests passed, 0 tests failed out of 48' in (REVIEW/'phase-5i-final-labs.log').read_text()
t=metrics['totals']
...
with tarfile.open(ROOT/'dist/packages/snodec-book-proposal-package.tar.gz') as archive:
    ...
    assert any(m.name.endswith('/ch22/CMakeLists.txt') for m in files)
result={'chapters':chapters,'part_viii_words':sum(c['words_after'] for c in chapters),'part_viii_budget':7400,
        ...
        'word_delta_from_phase_5h':t['total_words']-baseline['totals']['total_words']}
(REVIEW/'phase-5i-exit-checks.json').write_text(json.dumps(result,indent=2)+'\n')
print(json.dumps(result,indent=2))
print('PASS: Part VIII budgets, pedagogy, mappings, scope, preserved listings, clean final PDFs and package contents.')
print('Execution and editorial/visual review are recorded in separate Phase 5i logs; future Parts are not certified.')
