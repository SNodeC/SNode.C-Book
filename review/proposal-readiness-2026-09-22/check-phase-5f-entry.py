#!/usr/bin/env python3
"""Recheck the Part IV budgets, teaching contract, scope, preservation and artifacts."""
...
base = subprocess.check_output(['git', 'rev-parse', ':/proposal-readiness: phase 5d follow-up — relocate stranded Unix-socket introduction'], text=True).strip()
...
assert metrics == json.loads((REVIEW / 'metrics-after-phase-5e.json').read_text())
baseline = json.loads((REVIEW / 'metrics-after-phase-5d-follow-up.json').read_text())
...
allowed = {'manuscript/chapters/09-writing-socketcontext-classes-well.md',
           'manuscript/chapters/10-writing-socketcontextfactory-classes-well.md',
           'manuscript/chapters/11-building-the-same-protocol-over-different-lower-layers.md',
           'manuscript/parts/part-04-from-raw-connections-to-application-protocols.md'}
...
chapters=[]
for n,budget,headings in [(9,3200,8),(10,2800,7),(11,2800,7)]:
...
assert 'Part IV checkpoint' in (ROOT/'manuscript/chapters/11-building-the-same-protocol-over-different-lower-layers.md').read_text()
assert 'Part IV checkpoint' in (ROOT/'companion/exercises/ch11/README.md').read_text()
...
assert all(value == 0 for value in json.loads((REVIEW/'phase-5f-entry-results.json').read_text()).values())
assert '100% tests passed, 0 tests failed out of 28' in (REVIEW/'phase-5f-entry-labs.log').read_text()
t=metrics['totals']
...
with tarfile.open(ROOT/'dist/packages/snodec-book-proposal-package.tar.gz') as archive:
    ...
    assert any(m.name.endswith('/ch11/CMakeLists.txt') for m in files)
result={'chapters':chapters,'part_iv_words':sum(c['words_after'] for c in chapters),'part_iv_budget':8800,
        ...
        'word_delta_from_phase_5d_follow_up':t['total_words']-baseline['totals']['total_words']}
(REVIEW/'phase-5f-entry-exit-checks.json').write_text(json.dumps(result,indent=2)+'\n')
print(json.dumps(result,indent=2))
print('PASS: Part IV budgets, pedagogy, mappings, scope, preserved listings, clean final PDFs and package contents.')
print('Execution and editorial/visual review are recorded in separate Phase 5e logs; future Parts are not certified.')
