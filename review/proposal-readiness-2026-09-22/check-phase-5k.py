#!/usr/bin/env python3
"""Recheck the Part X budgets, teaching contract, scope, preservation and artifacts."""
...
base = subprocess.check_output(['git', 'rev-parse', ':/proposal-readiness: phase 5j — refine Part IX and verify persistence boundaries'], text=True).strip()
...
assert metrics == json.loads((REVIEW / 'metrics-after-phase-5k.json').read_text())
baseline = json.loads((REVIEW / 'metrics-after-phase-5j.json').read_text())
...
allowed = {'manuscript/chapters/25-cmake-components-and-linking-strategy.md', 'manuscript/chapters/26-deployment-on-linux-and-openwrt.md', 'manuscript/chapters/27-testing-debugging-and-benchmarking.md', 'manuscript/parts/part-10-building-porting-and-maintaining.md'}
...
assert not markers_before - markers_after
assert markers_after - markers_before == Counter({'<!-- snodec-source: companion/exercises/ch27/roundtrip.py -->': 1})
# One public calculation: the newly marked Python file is the unchanged printed listing.
benchmark_path = ROOT/'manuscript/chapters/27-testing-debugging-and-benchmarking.md'
benchmark = re.search(r'```python\n(.*?)\n```', benchmark_path.read_text(), re.S)[1]
assert (ROOT/'companion/exercises/ch27/roundtrip.py').read_text().rstrip() == benchmark
chapters=[]
for n,budget,headings in [(25,4100,10),(26,3700,10),(27,4200,10)]:
...
assert 'Part X checkpoint' in (ROOT/'manuscript/chapters/27-testing-debugging-and-benchmarking.md').read_text()
assert 'Part X checkpoint' in (ROOT/'companion/exercises/ch27/README.md').read_text()
...
regeneration=next(line for line in (ROOT/'build/proposal-readiness-phase-5k-examples/build.ninja').read_text().splitlines()
...
assert all(value == 0 for value in json.loads((REVIEW/'phase-5k-final-results.json').read_text()).values())
assert '100% tests passed, 0 tests failed out of 58' in (REVIEW/'phase-5k-final-labs.log').read_text()
t=metrics['totals']
...
with tarfile.open(ROOT/'dist/packages/snodec-book-proposal-package.tar.gz') as archive:
    ...
    assert any(m.name.endswith('/ch27/CMakeLists.txt') for m in files)
result={'chapters':chapters,'part_x_words':sum(c['words_after'] for c in chapters),'part_x_original_budget':11600,'part_x_budget':12000,
        ...
        'preservation_and_scope_checks_passed':True,'prior_lifetime_tests_preserved':True,'canonical_entry_dependencies_unique':True,'pdfs':pdfs,'archive_identical_files':len(files),
        'reserve_used':400,'reserve_remaining':2350,'chapter_27_original_budget':3800,'chapter_27_revised_budget':4200,'global_words':t['total_words'],
        'word_delta_from_phase_5j':t['total_words']-baseline['totals']['total_words']}
(REVIEW/'phase-5k-exit-checks.json').write_text(json.dumps(result,indent=2)+'\n')
print(json.dumps(result,indent=2))
print('PASS: Part X budgets, pedagogy, mappings, scope, preserved listings, clean final PDFs and package contents.')
print('Execution and editorial/visual review are recorded in separate Phase 5k logs; future Parts are not certified.')
