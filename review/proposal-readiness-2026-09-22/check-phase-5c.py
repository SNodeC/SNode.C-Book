#!/usr/bin/env python3
"""Recheck the Part II budgets, teaching contract, scope, preservation and artifacts."""
...
base = subprocess.check_output(['git', 'rev-parse', ':/proposal-readiness: phase 5b — refine Part I and establish the measurement checkpoint'], text=True).strip()
...
assert metrics == json.loads((REVIEW / 'metrics-after-phase-5c.json').read_text())
baseline = json.loads((REVIEW / 'metrics-after-phase-5b.json').read_text())
...
allowed = {'manuscript/chapters/04-the-mental-model-and-layers-in-practice.md',
           'manuscript/chapters/05-core-runtime-and-event-processing.md',
           'manuscript/parts/part-02-the-snodec-architecture.md',
           'manuscript/frontmatter/mainmatter.md'}
...
chapters=[]
for n,budget,headings in [(4,5300,15),(5,3700,11)]:
...
assert 'Part II checkpoint' in (ROOT/'manuscript/chapters/05-core-runtime-and-event-processing.md').read_text()
assert 'Part II checkpoint' in (ROOT/'companion/exercises/ch05/README.md').read_text()
...
# One page-bottom setting now governs TOC and body. No warning threshold is changed.
metadata='production/metadata/metadata.yaml'
assert (ROOT/metadata).read_text()==old(metadata).replace('    \\makeindex\n','    \\makeindex\n    \\raggedbottom\n')
main='manuscript/frontmatter/mainmatter.md'
assert (ROOT/main).read_text()==old(main).replace('\\raggedbottom\n','')
assert json.loads((REVIEW/'phase-5c-runtime-results.json').read_text())==dict.fromkeys(['labs','teaching','behavior','lifetime'],0)
assert '100% tests passed, 0 tests failed out of 16' in (REVIEW/'phase-5c-final-labs.log').read_text()
t=metrics['totals']
...
with tarfile.open(ROOT/'dist/packages/snodec-book-proposal-package.tar.gz') as archive:
    ...
    assert any(m.name.endswith('/ch05/deferred-work.cpp') for m in files)
result={'chapters':chapters,'part_ii_words':sum(c['words_after'] for c in chapters),'part_ii_budget':9000,
        ...
        'word_delta_from_phase_5b':t['total_words']-baseline['totals']['total_words']}
(REVIEW/'phase-5c-exit-checks.json').write_text(json.dumps(result,indent=2)+'\n')
print(json.dumps(result,indent=2))
print('PASS: Part II budgets, pedagogy, mappings, scope, preserved listings, clean final PDFs and package contents.')
print('Execution and editorial/visual review are recorded in separate Phase 5c logs; future Parts are not certified.')
