#!/usr/bin/env python3
"""Recheck the Part IX budgets, teaching contract, scope, preservation and artifacts."""
...
base = subprocess.check_output(['git', 'rev-parse', ':/proposal-readiness: phase 5i — refine Part VIII and verify MQTT delivery boundaries'], text=True).strip()
...
assert metrics == json.loads((REVIEW / 'metrics-after-phase-5j.json').read_text())
baseline = json.loads((REVIEW / 'metrics-after-phase-5i.json').read_text())
...
allowed = {'manuscript/chapters/23-database-support-and-application-state.md', 'manuscript/chapters/24-snodec-in-larger-systems.md', 'manuscript/parts/part-09-persistence-and-full-systems.md'}
...
chapters=[]
for n,budget,headings in [(23,3900,10),(24,6200,17)]:
...
assert 'Part IX checkpoint' in (ROOT/'manuscript/chapters/24-snodec-in-larger-systems.md').read_text()
assert 'Part IX checkpoint' in (ROOT/'companion/exercises/ch24/README.md').read_text()
...
regeneration=next(line for line in (ROOT/'build/proposal-readiness-phase-5j-examples/build.ninja').read_text().splitlines()
...
assert all(value == 0 for value in json.loads((REVIEW/'phase-5j-final-results.json').read_text()).values())
assert '100% tests passed, 0 tests failed out of 52' in (REVIEW/'phase-5j-final-labs.log').read_text()
t=metrics['totals']
...
with tarfile.open(ROOT/'dist/packages/snodec-book-proposal-package.tar.gz') as archive:
    ...
    assert any(m.name.endswith('/ch24/CMakeLists.txt') for m in files)
result={'chapters':chapters,'part_ix_words':sum(c['words_after'] for c in chapters),'part_ix_budget':10100,
        ...
        'word_delta_from_phase_5i':t['total_words']-baseline['totals']['total_words']}
(REVIEW/'phase-5j-exit-checks.json').write_text(json.dumps(result,indent=2)+'\n')
print(json.dumps(result,indent=2))
print('PASS: Part IX budgets, pedagogy, mappings, scope, preserved listings, clean final PDFs and package contents.')
print('Execution and editorial/visual review are recorded in separate Phase 5j logs; future Parts are not certified.')
