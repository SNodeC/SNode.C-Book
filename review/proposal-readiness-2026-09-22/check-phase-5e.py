...
assert all(value == 0 for value in json.loads((REVIEW/'phase-5e-final-results.json').read_text()).values())
assert '100% tests passed, 0 tests failed out of 28' in (REVIEW/'phase-5e-final-labs.log').read_text()
t=metrics['totals']
...
(REVIEW/'phase-5e-exit-checks.json').write_text(json.dumps(result,indent=2)+'\n')
