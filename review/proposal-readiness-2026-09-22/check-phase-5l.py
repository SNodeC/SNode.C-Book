#!/usr/bin/env python3
"""Recheck the Part XI budgets, teaching contract, scope, preservation and artifacts."""
...
base = subprocess.check_output(['git', 'rev-parse', ':/proposal-readiness: phase 5k — refine Part X and verify installed consumers'], text=True).strip()
...
assert metrics == json.loads((REVIEW / 'metrics-after-phase-5l.json').read_text())
baseline = json.loads((REVIEW / 'metrics-after-phase-5k.json').read_text())
...
allowed = {'manuscript/chapters/28-building-minigateway.md', 'manuscript/chapters/29-extending-minigateway-with-a-new-network-role.md', 'manuscript/chapters/30-architectural-judgment-choosing-the-right-layer-and-boundary.md', 'manuscript/parts/part-11-building-and-evaluating-minigateway.md'}
...
chapters=[]
for n,budget,headings in [(28,3850,6),(29,3150,6),(30,1900,6)]:
...
regeneration=next(line for line in (ROOT/'build/proposal-readiness-phase-5l-examples/build.ninja').read_text().splitlines()
...
assert all(value == 0 for value in json.loads((REVIEW/'phase-5l-final-results.json').read_text()).values())
assert '100% tests passed, 0 tests failed out of 60' in (REVIEW/'phase-5l-final-labs.log').read_text()
t=metrics['totals']
...
with tarfile.open(ROOT/'dist/packages/snodec-book-proposal-package.tar.gz') as archive:
    ...
    assert any(m.name.endswith('/ch29/CMakeLists.txt') for m in files)
result={'chapters':chapters,'part_xi_words':sum(c['words_after'] for c in chapters),'part_xi_budget':8900,
        ...
        'preservation_and_scope_checks_passed':True,'prior_lifetime_tests_preserved':True,'canonical_entry_dependencies_unique':True,'pdfs':pdfs,'archive_identical_files':len(files),'preserved_complete_markers':sum(markers_after.values()),'public_labs':60,
        'reserve_used_this_phase':0,'reserve_remaining':2350,'global_words':t['total_words'],
        'word_delta_from_phase_5k':t['total_words']-baseline['totals']['total_words']}
(REVIEW/'phase-5l-exit-checks.json').write_text(json.dumps(result,indent=2)+'\n')
print(json.dumps(result,indent=2))
print('PASS: Part XI budgets, pedagogy, mappings, scope, preserved listings, clean final PDFs and package contents.')
print('Execution and editorial/visual review are recorded in separate Phase 5l logs; future Parts are not certified.')
