#!/usr/bin/env python3
"""Recheck the Part V budgets, teaching contract, scope, preservation and artifacts."""
...
base = subprocess.check_output(['git', 'rev-parse', ':/proposal-readiness: phase 5e — refine Part IV and verify protocol framing'], text=True).strip()
...
assert metrics == json.loads((REVIEW / 'metrics-after-phase-5f.json').read_text())
baseline = json.loads((REVIEW / 'metrics-after-phase-5e.json').read_text())
...
allowed = {'manuscript/chapters/12-configuring-applications-and-named-instances.md',
           'manuscript/chapters/13-logging-diagnostics-and-runtime-introspection.md',
           'manuscript/parts/part-05-configuration-and-operational-behavior.md'}
...
chapters=[]
for n,budget,headings in [(12,5400,16),(13,2750,7)]:
...
assert 'Part V checkpoint' in (ROOT/'manuscript/chapters/13-logging-diagnostics-and-runtime-introspection.md').read_text()
assert 'Part V checkpoint' in (ROOT/'companion/exercises/ch13/README.md').read_text()
...
# Existing applications and chapter labs remain unchanged. The common harness extracts
# its existing environment construction so inspection and live peers use one implementation.
for path in subprocess.check_output(['git','ls-tree','-r','--name-only',base,'companion/examples','companion/exercises'],text=True).splitlines():
    if Path(path).suffix in {'.cpp','.h','.py'} and path != 'companion/exercises/lab_support.py': assert (ROOT/path).read_text()==old(path),path
...
assert all(value == 0 for value in json.loads((REVIEW/'phase-5g-entry-results.json').read_text()).values())
assert '100% tests passed, 0 tests failed out of 32' in (REVIEW/'phase-5g-entry-labs.log').read_text()
t=metrics['totals']
...
with tarfile.open(ROOT/'dist/packages/snodec-book-proposal-package.tar.gz') as archive:
    ...
    assert any(m.name.endswith('/ch13/CMakeLists.txt') for m in files)
result={'chapters':chapters,'part_v_words':sum(c['words_after'] for c in chapters),'part_v_budget':8150,
        ...
        'word_delta_from_phase_5e':t['total_words']-baseline['totals']['total_words']}
(REVIEW/'phase-5g-entry-exit-checks.json').write_text(json.dumps(result,indent=2)+'\n')
print(json.dumps(result,indent=2))
print('PASS: Part V budgets, pedagogy, mappings, scope, preserved listings, clean final PDFs and package contents.')
print('Execution and editorial/visual review are recorded in separate Phase 5f logs; future Parts are not certified.')
