#!/usr/bin/env python3
"""Recheck the Part VII budgets, teaching contract, scope, preservation and artifacts."""
...
base = subprocess.check_output(['git', 'rev-parse', ':/proposal-readiness: phase 5g follow-up — move TLS fixture into companion material'], text=True).strip()
...
assert metrics == json.loads((REVIEW / 'metrics-after-phase-5h.json').read_text())
baseline = json.loads((REVIEW / 'metrics-after-phase-5g-follow-up.json').read_text())
...
allowed = {'manuscript/chapters/16-the-http-layer.md', 'manuscript/chapters/17-the-express-like-framework.md', 'manuscript/chapters/18-server-sent-events-and-real-time-http.md', 'manuscript/chapters/19-websocket-and-protocol-upgrade.md', 'manuscript/parts/part-07-web-protocols-and-web-applications.md'}
...
chapters=[]
for n,budget,headings in [(16,2600,6),(17,2350,5),(18,3300,7),(19,3050,7)]:
...
assert 'Part VII checkpoint' in (ROOT/'manuscript/chapters/19-websocket-and-protocol-upgrade.md').read_text()
assert 'Part VII checkpoint' in (ROOT/'companion/exercises/ch19/README.md').read_text()
...
# The public WebSocket observer reuses every prior wire assertion and SSE check.
prior_lifetime = ast.parse(old('ci/run-example-lifetime-tests.py'))
current_lifetime = ast.parse((ROOT/'ci/run-example-lifetime-tests.py').read_text())
wire = ast.parse((ROOT/'companion/exercises/ch19/wire.py').read_text())
function = lambda tree, name: next(n for n in tree.body if isinstance(n, ast.FunctionDef) and n.name == name)
for name in ['exact', 'send_frame', 'frame', 'message']:
    assert ast.dump(function(prior_lifetime,name)) == ast.dump(function(wire,name))
assert ast.dump(function(prior_lifetime,'test_sse')) == ast.dump(function(current_lifetime,'test_sse'))
peer_block = next(n for n in ast.walk(function(prior_lifetime,'test_websocket'))
                  if isinstance(n,ast.With) and any(isinstance(i.optional_vars,ast.Name) and i.optional_vars.id=='peer' for i in n.items))
assert [ast.dump(n) for n in peer_block.body] == [ast.dump(n) for n in function(wire,'echo_exchange').body[1:]]
...
assert all(value == 0 for value in json.loads((REVIEW/'phase-5h-final-results.json').read_text()).values())
assert '100% tests passed, 0 tests failed out of 42' in (REVIEW/'phase-5h-final-labs.log').read_text()
t=metrics['totals']
...
with tarfile.open(ROOT/'dist/packages/snodec-book-proposal-package.tar.gz') as archive:
    ...
    assert any(m.name.endswith('/ch19/CMakeLists.txt') for m in files)
result={'chapters':chapters,'part_vii_words':sum(c['words_after'] for c in chapters),'part_vii_budget':11300,
        ...
        'preservation_and_scope_checks_passed':True,'prior_websocket_and_sse_assertions_preserved':True,'pdfs':pdfs,'archive_identical_files':len(files),
        'reserve_used':0,'reserve_remaining':2750,'global_words':t['total_words'],
        'word_delta_from_phase_5g_follow_up':t['total_words']-baseline['totals']['total_words']}
(REVIEW/'phase-5h-exit-checks.json').write_text(json.dumps(result,indent=2)+'\n')
print(json.dumps(result,indent=2))
print('PASS: Part VII budgets, pedagogy, mappings, scope, preserved listings, clean final PDFs and package contents.')
print('Execution and editorial/visual review are recorded in separate Phase 5h logs; future Parts are not certified.')
