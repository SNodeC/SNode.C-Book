...
root=Path.cwd(); tag=sys.argv[1]; build=root/'build'/sys.argv[2]; review=root/'review/proposal-readiness-2026-09-22'; prefix=root/'build/ci-fix-2026-09-22/install-gcc'; temp=root/'build/t'
assert temp.is_dir()
env=os.environ|{'SNODEC_PREFIX':str(prefix),'BOOK_EXAMPLES_BUILD_DIR':str(build),'CMAKE_BUILD_PARALLEL_LEVEL':'4','TMPDIR':str(temp),'TMP':str(temp),'TEMP':str(temp),'SNODEC_BOOK_SMOKE_UNIX_SOCKET':str(temp/'smoke.sock'),'PYTHONDONTWRITEBYTECODE':'1'}
