...
assert metrics == json.loads((REVIEW / 'metrics-after-phase-5g.json').read_text())
...
assert all(value == 0 for value in json.loads((REVIEW/'phase-5g-final-results.json').read_text()).values())
assert '100% tests passed, 0 tests failed out of 36' in (REVIEW/'phase-5g-final-labs.log').read_text()
t=metrics['totals']
...
(REVIEW/'phase-5g-exit-checks.json').write_text(json.dumps(result,indent=2)+'\n')
