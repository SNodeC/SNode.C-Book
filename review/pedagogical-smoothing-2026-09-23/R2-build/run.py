#!/usr/bin/env python3
"""Build and verify the R2 archive with unchanged assertions and timeouts."""
...
work=root/'build/rebaseline-R2-07ca9a29-followup11'
prefix=work/'install-gcc'
expected='07ca9a2936ee72582df7d159cb06666fe23e30f8'
...
def book_hashes():
 paths=subprocess.check_output(['git','ls-files','--cached','--others','--exclude-standard','-z','--','manuscript','ci','companion'],cwd=root).decode().split('\0')
 return {p:hashlib.sha256((root/p).read_bytes()).hexdigest() for p in sorted(set(paths)) if p and (root/p).is_file()}

before=freeze();assert before==json.loads((review.parent/'framework-freeze-R2.json').read_text())
assert before['head']==expected and before['status_porcelain_v1']=='',before
