...
C1 integration finding: the first matched-tool build fails at generated TeX line76 because metadata.yaml:38–39 calls `\clearcaptionsetup`, while the Pandoc3.9 template does not load the caption setup previously cleared. Removed that obsolete two-line override; no new package or compatibility path is needed. Rebuild and hosted validation cover the resulting toolchain.

C2/C5 PDF integration: physical page135 crowds the page number with the new Part V heading; use the existing `SNodeCNextSectionMark` facility, preserving the full heading and TOC. Physical page165 has a25.05pt inline filename overflow; put the intact filename sentence in its own paragraph. The main and proposal metadata both contained the obsolete caption override; both removals are verified by the matched-tool package build. These production corrections are committed as an additional C integration fix, because the initial A–D commits had already been pushed before PDF verification exposed them.
