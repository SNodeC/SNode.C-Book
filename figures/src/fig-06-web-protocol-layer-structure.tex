\documentclass[tikz,border=0pt]{standalone}
\input{snodec-figure-style.tex}
\usetikzlibrary{positioning,calc,arrows.meta,fit,backgrounds}

\begin{document}
\begin{tikzpicture}[
  snodec diagram,
  protocol block/.style={
    draw=snodecBlue!75,
    fill=snodecBlueSoft,
    rounded corners=2.5pt,
    line width=0.75pt,
    align=center,
    text width=92mm,
    inner xsep=3mm,
    inner ysep=2.4mm,
    font=\sffamily\small
  },
  dependent block/.style={
    draw=snodecBlue!65,
    fill=snodecBlueSoft!70,
    rounded corners=2.5pt,
    line width=0.75pt,
    align=center,
    text width=48mm,
    inner xsep=3mm,
    inner ysep=2.4mm,
    font=\sffamily\small
  },
  appstruct block/.style={
    draw=snodecOrange!75,
    fill=snodecOrangeSoft,
    rounded corners=2.5pt,
    line width=0.75pt,
    align=center,
    text width=58mm,
    inner xsep=3mm,
    inner ysep=2.4mm,
    font=\sffamily\small
  },
  app block/.style={
    draw=snodecGreen!75,
    fill=snodecGreenSoft,
    rounded corners=2.5pt,
    line width=0.75pt,
    align=center,
    text width=132mm,
    inner xsep=3mm,
    inner ysep=2.4mm,
    font=\sffamily\small
  },
  protocol flow/.style={
    snodec arrow,
    draw=snodecBlue!65
  },
  app flow/.style={
    snodec arrow,
    draw=snodecGreen!70
  },
  structure flow/.style={
    snodec arrow,
    draw=snodecOrange!75
  },
  link/.style={
    draw=snodecMuted!75,
    line width=0.75pt
  },
  arrow label/.style={
    font=\sffamily\footnotesize,
    text=snodecMuted,
    fill=white,
    inner sep=1.1pt
  }
]

\def\majorVGap{11mm}
\def\branchGap{6mm}

% ----------------------------------------------------------------------
% Web protocol layer
% ----------------------------------------------------------------------
\node[protocol block] (http) {%
  \textbf{\textcolor{snodecBlue}{HTTP request/response}}\\[-0.4mm]
  {\footnotesize requests, responses, headers, body, lifecycle, upgrade path}
};

\node[dependent block, below=13mm of http, xshift=-34mm] (sse) {%
  \textbf{\textcolor{snodecBlue}{Server-Sent Events}}\\[-0.4mm]
  {\footnotesize long-lived HTTP response}\\[-0.2mm]
  {\footnotesize server $\rightarrow$ client events}
};

\node[dependent block, below=13mm of http, xshift=34mm] (ws) {%
  \textbf{\textcolor{snodecBlue}{WebSocket}}\\[-0.4mm]
  {\footnotesize HTTP upgrade, then frames}\\[-0.2mm]
  {\footnotesize client $\leftrightarrow$ server}
};

\coordinate (protocolBranch) at ([yshift=\branchGap]$(sse.north)!0.5!(ws.north)$);
\coordinate (sseBranch)      at ([yshift=\branchGap]sse.north);
\coordinate (wsBranch)       at ([yshift=\branchGap]ws.north);

\draw[link] (http.south) -- (protocolBranch);
\draw[link] (sseBranch) -- (wsBranch);
\draw[protocol flow] (sseBranch) -- (sse.north);
\draw[protocol flow] (wsBranch) -- (ws.north);

\begin{scope}[on background layer]
  \node[
    fit=(http)(sse)(ws)(protocolBranch)(sseBranch)(wsBranch),
    snodec boundary,
    inner sep=5mm,
    label={[font=\sffamily\bfseries\small,text=snodecBlue,fill=white,inner sep=1.2pt]above:Web protocol layer}
  ] (webLayer) {};
\end{scope}

% ----------------------------------------------------------------------
% Express-like application structure
% ----------------------------------------------------------------------
\coordinate (expressAnchor) at ([yshift=-\majorVGap]webLayer.south -| sse.center);

\node[
  appstruct block,
  anchor=north
] (express) at (expressAnchor) {%
  \textbf{\textcolor{snodecOrange}{Express-like application structure}}\\[-0.4mm]
  {\footnotesize routing, middleware, mounted handlers}\\[-0.2mm]
  {\footnotesize organizes routes, SSE endpoints, WS upgrades}
};

\coordinate (expressToWebEnd) at (webLayer.south -| sse.center);

\draw[structure flow]
  (express.north)
  -- node[arrow label, right=1.5mm] {uses}
  (expressToWebEnd);

% ----------------------------------------------------------------------
% Application code
% ----------------------------------------------------------------------
\node[
  app block,
  below=\majorVGap of express.south west,
  anchor=north west
] (app) {%
  \textbf{\textcolor{snodecGreen}{Application code}}\\[-0.4mm]
  {\footnotesize APIs, pages, dashboards, monitoring, commands, sessions}
};

% Application uses the Express-like structure.
\coordinate (appToExpressStart) at (app.north -| sse.center);
\coordinate (appToExpressEnd)   at (express.south -| sse.center);

\draw[app flow]
  (appToExpressStart)
  -- node[arrow label, right=1.5mm] {uses}
  (appToExpressEnd);

% Application can also use the web protocol layer directly.
\coordinate (directStart) at (app.north -| ws.center);
\coordinate (directEnd)   at (webLayer.south -| ws.center);

\draw[app flow]
  (directStart)
  -- node[arrow label, right=1.5mm] {direct use}
  (directEnd);


% ----------------------------------------------------------------------
% Fixed 160 mm output canvas.
% The drawing stays at native size and is centered horizontally.
% Vertical height remains natural, with the former standalone 7pt margin
% preserved above and below.
% ----------------------------------------------------------------------
\coordinate (snodecCanvasCenter) at (current bounding box.center);
\coordinate (snodecCanvasSW)
  at ($(snodecCanvasCenter)+(-80mm,0)$ |- current bounding box.south);
\coordinate (snodecCanvasNE)
  at ($(snodecCanvasCenter)+(80mm,0)$ |- current bounding box.north);
\path[use as bounding box]
  ([yshift=-7pt]snodecCanvasSW)
  rectangle
  ([yshift=7pt]snodecCanvasNE);

\end{tikzpicture}
\end{document}
