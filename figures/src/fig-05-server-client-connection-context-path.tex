...
\begin{tikzpicture}[
  ...
  runtime flow/.style={snodec arrow, draw=snodecBlue!65},
  link/.style={draw=snodecMuted!75, line width=0.75pt}
]

% --- application side ----------------------------------------------------
\node[app box] (handle) {%
  \textbf{\textcolor{snodecGreen}{Application-side handle}}\\[-0.4mm]
  {\footnotesize \texttt{SocketServer} / \texttt{SocketClient} visible in user code}
};

\node[app box, below=7mm of handle] (registration) {%
  \textbf{\textcolor{snodecGreen}{Registration call}}\\[-0.4mm]
...
\node[split box, left=7mm of splitCenter] (server) {%
  \textbf{\textcolor{snodecBlue}{Server-side instance}}\\[-0.4mm]
  {\footnotesize listens and accepts peers}
};

\node[split box, right=7mm of splitCenter] (client) {%
  \textbf{\textcolor{snodecBlue}{Client-side instance}}\\[-0.4mm]
...
\node[runtime box, below=12mm of splitBottom] (machinery) {%
  \textbf{\textcolor{snodecBlue}{Accept/connect machinery}}\\[-0.4mm]
