\documentclass[tikz,border=0pt]{standalone}
\input{snodec-figure-style.tex}
\usetikzlibrary{calc}

\begin{document}
\begin{tikzpicture}[
  snodec diagram,
  app block/.style={
    draw=snodecGreen!75,
    fill=snodecGreenSoft,
    rounded corners=\snodecBoxCorner,
    line width=\snodecBoxLineWidth,
    align=left,
    text width=92mm,
    inner xsep=4.5mm,
    inner ysep=2.6mm,
    minimum height=11mm
  },
  runtime block/.style={
    draw=snodecBlue!75,
    fill=snodecBlueSoft,
    rounded corners=\snodecBoxCorner,
    line width=\snodecBoxLineWidth,
    align=left,
    text width=92mm,
    inner xsep=4.5mm,
    inner ysep=2.6mm,
    minimum height=11mm
  },
  flow/.style={snodec main arrow, draw=snodecBlue!65},
  register flow/.style={snodec main arrow, draw=snodecGreen!70},
  arrow label/.style={snodec arrow label}
]

\node[app block] (appcode) {\textbf{\textcolor{snodecGreen}{Application code}}\\[-0.5mm]
  {\footnotesize Creates and configures communication roles.}
};

\node[app block, below=\snodecGapSmall of appcode] (handle) {\textbf{\textcolor{snodecGreen}{Application-side handle}}\\[-0.5mm]
  {\footnotesize Visible \texttt{SocketServer} or \texttt{SocketClient} object.}
};

\node[runtime block, below=\snodecGapLarge of handle] (instance) {\textbf{\textcolor{snodecBlue}{Instance}}\\[-0.5mm]
  {\footnotesize Registered runtime communication role.}
};

\node[runtime block, below=\snodecGapSmall of instance] (connection) {\textbf{\textcolor{snodecBlue}{Connection}}\\[-0.5mm]
  {\footnotesize Concrete peer relationship.}
};

\node[runtime block, below=\snodecGapSmall of connection] (factory) {\textbf{\textcolor{snodecBlue}{Context factory}}\\[-0.5mm]
  {\footnotesize Creates contexts for connections.}
};

\node[runtime block, below=\snodecGapSmall of factory] (context) {\textbf{\textcolor{snodecBlue}{Context}}\\[-0.5mm]
  {\footnotesize Per-connection protocol behavior.}
};

\draw[register flow] (appcode.south) -- (handle.north);
\draw[register flow] (handle.south) -- (instance.north);
\draw[flow] (instance.south) -- (connection.north);
\draw[flow] (connection.south) -- (factory.north);
\draw[flow] (factory.south) -- (context.north);

\begin{scope}[on background layer]
  \node[
    fit=(instance)(connection)(factory)(context),
    snodec boundary,
    inner sep=\snodecBoundaryInnerSep
  ] (runtime) {};
\end{scope}

% Foreground overlay title: masks the dashed border and any arrow below it.

\node[snodec boundary title] at (runtime.north) {core::SNodeC runtime};

\snodecApplyFixedCanvas

\end{tikzpicture}
\end{document}
